...
Update with files As you can see we provide 3 Verbs for the model update (PUT, PATCH, POST), the reason of that
...
`$resource` property In the previous example we have used the `$this->resource` call, well, keep in mind, that you
...
Specific fields Similar with the specific fields for the `index` request, you can define methods to return
...
Fillable Restify will fill your model attributes (defined in the `fields` method) even if they are listed
as `$guarded`.
...
Non wrapped route Clean routes if `$wrap` is false, your routes will have any Route group `$attributes`, that
