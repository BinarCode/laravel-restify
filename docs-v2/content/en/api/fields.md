...
If you omit the attribute name and pass only a callback (`field(fn () => ...)`), you get a *label-less* computed field: it has no column to read or write, so it is not writeable. Give it `->label('key')` - every label-less computed field otherwise serializes under the same `Computed` key, and if you declare more than one, the last one resolved silently wins.
