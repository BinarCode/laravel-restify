...
If you omit the attribute name and pass only a callback (`field(fn () => ...)`), you get a *label-less* computed field: it has no column to read or write, so it is not writeable. It also has no natural key, so Restify numbers it positionally: the first label-less computed field on a repository serializes under `Computed`, the second under `Computed_1`, the third under `Computed_2`, and so on, following declaration order. A field - computed or not - that is already labeled or named `Computed` or `Computed_N` reserves that number, so later label-less computed fields skip past it. This keeps the same field resolving to the same key on every request *for a given set of declared fields*, but the key still shifts if you add, remove or reorder fields - including conditionally, via `$this->when(...)`, `mergeWhen(...)`, or branching on the authenticated user inside `fields()` - so `->label('key')` any computed field a frontend actually reads.
...
```php
$field = field(fn () => "$this->first_name $this->last_name");
$isComputed = $field->computed(); // Returns: true
```

`field('full_name', fn () => ...)` is not computed - the second argument there is a resolve callback for an existing `full_name` attribute, so `computed()` returns `false`.
