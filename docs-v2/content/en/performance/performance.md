...
The `ttl` value is read as seconds: an int, or a numeric string (as `env()` returns it). Any
other value - `null`, a non-numeric string, a `Carbon` instance, a `DateInterval`, etc. -
disables caching for that request instead of caching forever, so a misconfigured `ttl`
fails safe.
...
`$cacheStore` takes precedence over `restify.repositories.cache.store` when both are set. When `$cacheStore` is `null` (the default), the repository falls back to `restify.repositories.cache.store` / `RESTIFY_REPOSITORY_CACHE_STORE`; when that is also empty or unset, it uses your application's default cache store (`config('cache.default')`).
