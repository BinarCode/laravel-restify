...
`cache:clear`, `config:clear`, `view:clear`) - `cache:clear` flushes your
app's default cache store (`cache.default`). If `restify.repositories.cache.store`
points at a different store than the default, `restify:refresh` also flushes
that store, so repository caching does not need a separate command when it
lives on its own store.
