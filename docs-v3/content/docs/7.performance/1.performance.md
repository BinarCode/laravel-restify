...
repository, which clears each one on the store it actually uses, by tags
when the store supports them - so a repository caching to redis while the
app default is file still gets cleared.
