...
description: Boost Laravel Restify API speed with policy caching, index meta toggling, and repository index caching to handle large datasets efficiently.
...
```bash
php artisan restify:refresh
```

This clears the route, application, config and view caches (`route:clear`,
`cache:clear`, `config:clear`, `view:clear`) - `cache:clear` only flushes your
app's default cache store (`cache.default`). Because a repository can cache
to a store of its own (see [Repository-Specific Configuration](#repository-specific-configuration)
above), `restify:refresh` also calls `clearCache()` on every registered
repository - but only while repository caching is enabled
(`restify.repositories.cache.enabled`); if it's disabled, `clearCache()` is a
no-op and stale entries are left in place. When it runs, each repository is
cleared on the store it actually uses, by tags when the store supports them
(redis, memcached, the array store) - so a repository caching to redis while
the app default is file still gets cleared without touching unrelated keys
on that store. On a store that doesn't support tagging (the database or
file drivers), there's no way to target only that repository's entries, so
`clearCache()` falls back to flushing the **entire store** - which can
include data Restify never wrote if that store is shared with the rest of
the app.
