---
title: Repositories
description: Learn how Laravel Restify repositories power your REST API and AI agent integrations, with Artisan commands, field definitions, and lifecycle control.
category: API
---
...
We highly recommend avoiding this kind of exposure. If you need to expose custom data, you can use the [serializer](/docs/api/serializer) to return a json:api format from any custom route/controller (still using the power of repositories).
...
Field class has many mutations, validators and interactions that you can use. These are documented [here](/docs/api/fields)
...
From Restify 7+, the meta on index requests will not be loaded anymore due to performance reasons. See [index item meta](/docs/api/repositories#index-item-meta) for more details.
...
The payload for a bulk update should contain an array of objects and each object must contain an `id` key - the
model's route key (its primary key by default). This `id` is required and must be unique across the rows in the
same request; a row missing it, or repeating an `id` another row in the payload already uses, gets a `422` naming
the offending row (e.g. `1.id`) and nothing in the request is written. Based on the `id`,
...
These models will be resolved from the database and checked for the `deleteBulk` policy permission. If any of the models are not allowed to be deleted, no entries will be deleted. Every entry in the array is required - a
missing or `null` entry gets a `422` naming its index (e.g. `keys.1`) instead of deleting anything.

## Force eager loading

Although Laravel Restify [provides eager](/docs/search/basic-filters) loading based on the query `related` property, you may want to force
