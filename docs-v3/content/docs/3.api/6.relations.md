---
title: Relations
description: "Learn how Restify handles Eloquent relationships. Define HasMany, BelongsTo, and other eager fields in the related() method to expose nested API resources."
category: API
---
...
Starting with Restify 7+, meta information for related (in index requests) will not be displayed. For more details read the [repository meta](/docs/api/repositories#index-item-meta).
...
The `BelongsTo` field allows you to use the search endpoint to [search over a column](/docs/search/basic-filters#repository-search) from the `belongsTo` relationship by simply using the `searchables` call: 
...
main index pagination (see [Pagination](/docs/api/repositories-basic#pagination)) - a value above
the cap is clamped down to it, never rejected. Leave the config `null`, or set it to `0`, an
empty string, a negative number, or any other non-numeric value, to mean "no cap".
