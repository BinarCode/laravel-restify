...
the cap is clamped down to it, never rejected.

<alert type="warning">

This cap (and `relatablePerPage` itself) only takes effect on `show` - each related field
resolves its own limited query there. On `index`, `?related=posts` is eager-loaded once for
every row on the page via a plain Eloquent `with()`, which has no per-relation limit, so
`relatablePerPage` and `max_per_page` are not applied.
