...
description: "Learn how Laravel Restify repositories expose data as MCP tools using the HasMcpTools trait, enabling AI agents to query, filter, and sort records efficiently."
category: MCP
---
...
The MCP index tool goes through the same index logic as the REST endpoint, so
`restify.pagination.max_per_page` (see [Pagination](/docs/api/repositories-basic#pagination))
clamps a `perPage` requested by an AI agent exactly the same way it clamps a REST client's.
...
For detailed information about optimizing field responses for AI agents, including MCP-specific field methods, token optimization, and conditional field visibility, see the **[MCP Fields](/docs/mcp/fields)** documentation.
