...
description: "Set up authentication in Laravel Restify using Sanctum — register, login, logout, password reset, and email verification with one artisan command."
category: Auth
---
...
- **Adds** the `HasApiTokens` trait to your `User` model, if it's missing
- **Appends** the `Route::restifyAuth();` line to the `routes/api.php` file to add the authentication routes

Under the hood, the route-append step is its own command, `php artisan restify:auth-macro`, which you can also run on its own if you only need the `routes/api.php` line appended.
...
We will discuss authorization in more detail in the [Authorization](/docs/auth/authorization) section. For now, let's see a simple example. 
...
**Learn More**: Check the [MCP Server Guide](/docs/mcp/mcp) for detailed AI agent configuration.
